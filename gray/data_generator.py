...
def gen_patches(file_name, mode):
        ...
        for i in range(0, h_scaled-patch_size+1, stride):
            for j in range(0, w_scaled-patch_size+1, stride):
                ...
                for k in range(0, aug_times):
                    x_aug = data_aug(x, mode=np.random.randint(start, 8))
                    patches.append(x_aug)
    return patches


def datagenerator(data_dir='data/Train400', verbose=False, mode='S', channels=1):
    ...
    for i in range(len(file_list)):
        patches = gen_patches(file_list[i], mode)
